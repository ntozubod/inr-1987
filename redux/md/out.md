...
continued to relations of any degree.
...
must read exactly one character (or end-marker) from tape zero. They can
only write and the choice is determined. Note also that explicit
end-markers are introduced. The existence of explicit end-markers can
confuse some of the operations that use the characterizing language
idea. It removes explicit end-markers from any tape.
