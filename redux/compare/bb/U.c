...
int U_member ( U_OBJECT U, int reca, int recb, int recc )
{
  ...
  while ( * p < MAXSHORT ) {
    ...
    if ( U -> U_rec [ * p ] . A_a == reca && U -> U_rec [ * p ] . A_b == recb && U -> U_rec [ * p ] . A_c == recc ) {
      return ( * p ) ;
    }
...
U_OBJECT U_grow ( U_OBJECT U, int lrec )
{
  ...
  Sfree ( ( char * ) U -> U_hash ) ;
  U -> U_rec = ( A_row * ) Srealloc ( ( char * ) U -> U_rec, lrec * sizeof ( A_row ) ) ;
  ...
  while ( q < ql ) {
    if ( U_member ( U, ( int ) q -> A_a, ( int ) q -> A_b, ( int ) q -> A_c ) != ( - 1 ) ) {
      Error ( "U_grow: BOTCH" ) ;
    }
