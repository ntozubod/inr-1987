  ...
  for ( current = 0 ;
    ...
    while ( p1 < p1z || p2 < p2z ) {
      ...
      if ( p1 -> A_b == 1 || p2 -> A_b == 1 ) {
        ...
        if ( flag && s2 > 0 ) {
          A = A_add ( A, current, ( A -> A_nT == 1 ? s2 : ( A -> A_nT == 2 ? ( s2 << 1 ) : s2 * A -> A_nT ) ) + t2 + A1 -> A_nT - 2, U_insert ( U, cur_a, ( int ) p2 -> A_c, 1 ) ) ;
        }
        ...
        A = A_add ( A, current, 0, U_insert ( U, ( int ) p1 -> A_c, ( int ) p2 -> A_c, 1 ) ) ;
        ++ p1 ;
        ++ p2 ;
        s1 = s2 = ( - 1 ) ;
