...
A_OBJECT A_LMsseq ( A_OBJECT A )
{
  ...
  if ( A == NULL ) {
    Error ( "A_sseq: No OBJECT" ) ;
  }

  if ( A -> A_nQ >= LAST ) {
    Error ( "A_sseq: Too many states" ) ;
  }
  ...
  for ( i = 0 ;
    ...
    for ( p = A -> A_p [ queue [ i ] ] ;
      ...
      if ( p -> A_b != 1 && p -> A_b % 2 == 1 ) {
        ...
        len = veclen ( to_coeff [ p -> A_a ] ) + 1 - st_len [ p -> A_a ] + st_len [ p -> A_c ] ;
        to_coeff [ p -> A_c ] = s_alloc ( len + 1 ) ;
        veccpy ( to_coeff [ p -> A_c ], to_coeff [ p -> A_a ] ) ;

        if ( st_len [ p -> A_a ] == 0 ) {
          to_coeff [ p -> A_c ] [ len - st_len [ p -> A_c ] - 1 ] = p -> A_b / 2 ;
          veccpy ( to_coeff [ p -> A_c ] + ( len - st_len [ p -> A_c ] ), st_ptr [ p -> A_c ] ) ;
        }

        if ( save_coeff ) {
          if ( veccmp ( save_coeff, to_coeff [ p -> A_c ] ) && veclen ( save_coeff ) == veclen ( to_coeff [ p -> A_c ] ) ) {
  ...
  if ( V_insert ( V, vec ) != START ) {
    Error ( "A_sseq: BOTCH 1" ) ;
  }

  current = START ;

  if ( V_insert ( V, vec + vlen + vlen ) != FINAL ) {
    Error ( "A_sseq: BOTCH 2" ) ;
  }
    ...
    for ( ;
      ...
      if ( ( last -> A_b != heap [ 1 ] -> A_b || hsize == 0 ) && vlen > 0 ) {
        ...
        if ( to == tmp ) {
          ...
          if ( to >= sig_lim ) {
            sig_lim *= 2 ;
            sig = ( SHORT * ) Srealloc ( ( char * ) sig, sig_lim * sizeof ( SHORT ) ) ;
            back = ( SHORT * ) Srealloc ( ( char * ) back, sig_lim * sizeof ( SHORT ) ) ;
          }
      ...
      if ( bb == 1 || bb % 2 == 0 ) {
        ...
        if ( st_len [ aa ] < st_len [ cc ] ) {
          veccpy ( to_coeff [ cc ] + veclen ( to_coeff [ cc ] ), st_ptr [ cc ] + st_len [ aa ] ) ;
        }
        ...
        if ( save_coeff ) {
          if ( veccmp ( save_coeff, to_coeff [ cc ] ) && veclen ( save_coeff ) == veclen ( to_coeff [ cc ] ) ) {
              ...
              if ( p -> A_b != 1 && p -> A_b % 2 == 1 ) {
                ...
                if ( save_coeff ) {
                  if ( veccmp ( save_coeff, to_coeff [ p -> A_c ] ) && veclen ( save_coeff ) == veclen ( to_coeff [ p -> A_c ] ) ) {
                    ...
                    for ( k = 0 ;
                          save_coeff [ k ] < MAXSHORT ;
                          k ++ ) {
                      fprintf ( fpout, "%s ", T_name ( TT, save_coeff [ k ] ) ) ;
                    }
