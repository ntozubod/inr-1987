...
A_OBJECT A_intersect ( A_OBJECT A1, A_OBJECT A2 )
{
  ...
  if ( A -> A_nT > 1 ) {
    fprintf ( fpout, "Warning: & applied to multi-tape automaton\n" ) ;
  }
  ...
  for ( current = 0 ;
    ...
    while ( p1 < p1z && p2 < p2z ) {
      if ( p1 -> A_b < p2 -> A_b ) {
        ...
        A = A_add ( A, current, ( int ) p1 -> A_b, R_insert ( R, ( int ) p1 -> A_c, ( int ) p2 -> A_c ) ) ;
        ++ p1 ;
        ++ p2 ;
      }
    }
...
A_OBJECT A_shuffle ( A_OBJECT A1, A_OBJECT A2 )
{
  ...
  for ( current = 0 ;
    ...
    if ( p1 < p1z && p2 < p2z && p1 -> A_b == 1 && p2 -> A_b == 1 ) {
    ...
    while ( p1 < p1z ) {
      A = A_add ( A, current, ( int ) p1 -> A_b, R_insert ( R, ( int ) p1 -> A_c, cur_b ) ) ;
      ++ p1 ;
    }

    while ( p2 < p2z ) {
      A = A_add ( A, current, ( int ) p2 -> A_b, R_insert ( R, cur_a, ( int ) p2 -> A_c ) ) ;
      ++ p2 ;
    }
  }
