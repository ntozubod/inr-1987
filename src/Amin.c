...
register A_OBJECT A;
{
        ...
        if ( A_report ) fprintf( fpout, "--> A_min\n" );
        A = A_open( A );
        ...
        for( s = A-> A_nQ; --s >= 0; ) int_N[ s ] = UNMARK;
        ...
        while ( wait_H < LAST ) {
                ...
                hsize = 0;
                p = NULL;
                for( s = B_H[b]; s != B_H[b] || p == NULL; s = B_N[s] ) {
                        p = lo = A-> A_p[ s ];
                        hi = A-> A_p[ s + 1 ] - 1;
                        if ( lo <= hi && x <= hi-> A_b ) {
                                while ( lo < hi ) {
                ...
                for( j = hsize / 2; j > 0; --j ) {
                        lo = heap[ j ];
                        for( f = j; (s = 2 * f) <= hsize; f = s ) {
                                if ( s < hsize
                                  && heap[s]-> A_b > heap[s+1]-> A_b ) ++s;
                                if ( lo-> A_b <= heap[s]-> A_b ) break;
                                heap[f] = heap[s];
                        }
                        heap[f] = lo;
                }
...
if ( x != heap[1]-> A_b || hsize == 0 ) {
        ...
        for( j = JL_H; j < LAST; j = JL_N[j] ) {
                if ( in_JL[ j ] < B_card[ j ] ) {
                        ...
                        for( s = B_H[nB]; (ns = B_N[s]) < LAST; s = ns )
                                B_L[ ns ] = s;
                        B_L[ B_N[s] = B_H[nB] ] = s;
                        in_JL[ nB ] = 0;
                        if ( in_wait[ j ] < LAST
...
for( f = 1; (s = 2 * f) <= hsize; f = s ) {
        if ( s < hsize && heap[s]-> A_b > heap[s+1]-> A_b ) ++s;
        if ( lo-> A_b <= heap[s]-> A_b ) break;
        heap[f] = heap[s];
}
heap[ f ] = lo;
