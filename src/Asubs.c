...
register A_OBJECT A;
{
        ...
        if ( A == NULL ) Error( "A_subs: No OBJECT" );
        if ( A-> A_nQ >= LAST ) Error( "A_subs: Too many states" );
        if ( A-> A_mode <  NFA_CLOSED ) A = A_clsure( A );
        if ( A-> A_mode >= DFA ) return( A );
        if ( A-> A_nrows == 0 ) {
                A-> A_mode = DFA;
                return( A );
        }

        if ( A_report ) fprintf( fpout, "--> A_subs\n" );
        set = s_alloc( A-> A_nQ );
        ...
        for( i = A-> A_nQ; --i >= 0; ) {
                p  = A-> A_p[ i ];
                pz = A-> A_p[ i + 1 ];
                while( p < pz && p-> A_b == 0 ) ++p;
                pnlam[ i ] = p;
        }

        vec[ i = 0 ] = START;
        pz = pnlam[ START ];
        for( p = A-> A_t; p < pz; ++p ) vec[ ++i ] = p-> A_c;
        vec[ i + 1 ] = MAXSHORT;
        if ( V_insert( V, vec ) != START ) Error( "A_subs: BOTCH 1" );

        vec[ 0 ] = FINAL;
        vec[ 1 ] = MAXSHORT;
        if ( V_insert( V, vec ) != FINAL ) Error( "A_subs: BOTCH 2" );

        for( current = 0; current < V-> V_n; current++ ) {
                hsize = 0;
                fvec = V_vec( V, current );
                for( i = 0; (j = fvec[i]) < MAXSHORT; i++ )
                        if ( pnlam[ j ] != A-> A_p[ j + 1 ] )
                                heap[ ++hsize ] = pnlam[ j ];
                if ( hsize == 0 ) continue;
                for( base = hsize / 2; base > 0; --base ) {
                        insert = heap[ base ];
                        for( father = base; (son = 2 * father) <= hsize;
                                        father = son ) {
                                if ( son < hsize
                ...
                for(;;) {
                        if ( last-> A_b != heap[1]-> A_b || hsize == 0 ) {
                                ...
                                vlen = 0;
                                vec[ n ] = MAXSHORT;
                                An = A_add( An, current, (int)last-> A_b,
                                        V_insert( V, vec ) );
                                if ( hsize == 0 ) break;
                        }
                        i = heap[1]-> A_c;
                        ...
                        last = heap[1];
                        for( father = 1; (son = 2 * father) <= hsize;
                                        father = son ) {
                                if ( son < hsize
                                  && heap[son]-> A_b > heap[son+1]-> A_b )
                                                son++;
