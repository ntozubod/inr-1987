...
A_OBJECT A_clsseq( A_OBJECT A1 )
{
    ...
    vec[0] = START;
    vec[1] = MAXSHORT;
    if ( V_insert( V, vec ) != START ) Error( "A_clsseq: BOTCH 1" );

    vec[0] = FINAL;
    vec[1] = FINAL;
    vec[2] = MAXSHORT;
    if ( V_insert( V, vec ) != FINAL ) Error( "A_clsseq: BOTCH 2" );
    ...
    for( current = 0; current < V-> V_n; current++ ) {
        ...
        */
            ...
            i = 0;
            k = 0;
            if ( label != 1 ) k = 2 * label;
            A = A_add( A, current, k, hi_next-- );
            if ( hi_next < V-> V_n ) Error( "A_clsseq: Too many states" );
            ...
            while( i >= 0 ) {
                ...
                if ( lo > hi || lo-> A_b == 1 || lo -> A_b % 2 == 0 ) {
                    if ( label ) {
                        ...
                        if ( i >= 98 ) Error( "A_clsseq: State explosion" );
                        if ( vec[i+1] == MAXSHORT ) {
                            vec[i+1] = end_st;
                            vec[i+2] = MAXSHORT;
                        }
                    ...
                    vec[i] = lo-> A_c;
                    ++i;
                    if ( i >= 100 ) Error( "A_clsseq: State explosion" );
                    label = lo-> A_b / 2;
                    if ( vec[i] == MAXSHORT ) {
                        if ( i >= 2 && vec[i-1] == vec[i-2]
                                && label == last_label ) {
                            k = 0;
                            if ( label != 1 ) k = 2 * label + 1;
