...
register A_OBJECT A;
{
    ...
    label = 0;
    from = 0; /* Initialiaze to suppress warning JHJ */
    int GMval;
        ...
        for( p = A-> A_p[ queue[i] ]; p < pz; ++p ) {
            if ( p-> A_b != 1 && p-> A_b % 2 == 1 ) {
                ...
                if ( set[ p-> A_c ] == UNMARK ) {
                    ...
                    save_coeff = 0;
                    queue[ nq++ ] = p-> A_c;
                    if ( nq >= queue_lim ) Error( "queue_lim exceeded (1)" );
                } else {
                ...
                if ( save_coeff ) {
                    if ( GMcmp( save_coeff, to_coeff[p->A_c] ) >= 0 ) {
                        ...
                        queue[ nq++ ] = p-> A_c;
                        if ( nq >= queue_lim ) Error( "queue_lim exceeded (2)" );
                    } else {
                        ...
                        for( p = A-> A_p[ queue[i] ]; p < pz; ++p ) {
                            if ( p-> A_b != 1 && p-> A_b % 2 == 1 ) {
                                ...
                                if ( set[ p-> A_c ] == UNMARK ) {
                                    ...
                                    save_coeff = 0;
                                    queue[ nq++ ] = p-> A_c;
                                    if ( nq >= queue_lim ) Error( "queue_lim exceeded (3)" );
                                } else {
                                    save_coeff = to_coeff[ p-> A_c ];
                                }
                                len = veclen( to_coeff[ p-> A_a ] ) + 1
                                ...
                                if ( save_coeff ) {
                                    if ( GMcmp( save_coeff,
                                        ...
                                        Sfree( (char *) save_coeff );
                                        queue[ nq++ ] = p-> A_c;
                                        if ( nq >= queue_lim ) Error( "queue_lim exceeded (4)" );
                                    } else {
                                        Sfree( (char *) to_coeff[ p-> A_c ] );
                                        to_coeff[p-> A_c] = save_coeff;
                                    }
