...
A_OBJECT A_retape( A_OBJECT A1, A_OBJECT A2, Tn_OBJECT Tn )
{
    int i;
    A_row *p;
    ...
    for( i = A1-> A_nrows; --i >= 0; ) {
        ...
        if ( sigma == 1 || tp[ tape ] == tp[ tape + 1 ] )
            ...
            if ( i1 < i2 ) {
                ...
                p-> A_c = base + i1;
                while ( ++i1 < i2 )
                    A1 = A_add( A1, base + i1 - 1,
                                sigma * ntapes + trans[ i1 ],
                                base + i1 );
                A1 = A_add( A1, base + i1 - 1,
                            sigma * ntapes + trans[ i1 ],
                            old );
