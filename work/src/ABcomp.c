...
A_OBJECT AB_comp_1( A_OBJECT A1, B4_OBJECT B2, T2_OBJECT T2_Sigma )
{
    A_OBJECT A;
    R_OBJECT R1;
    R_OBJECT R2;
    int current, cur_a, cur_b, cur_c, next_c, s1;
...
/* printf( "%d %d %d\n", cur_a, cur_b, cur_c ); */
        ...
        while( p1 < p1z ) {
            s1 = p1-> A_b;
/* printf( "A: %d %d %d\n", p1-> A_a, p1-> A_b, p1-> A_c ); */
            B2 = B4_set_trans( B2, cur_b, s1, T2_Sigma );
            if ( B2-> B4_to != MAXSHORT ) {
                next_c =
                    R_insert( R2, R_insert( R1, p1-> A_c, B2-> B4_to ), 0 );
                ...
                if ( output1 == MAXSHORT ) {
                    ...
                    while ( output2 != MAXSHORT ) {
                        ...
                        A = A_add( A, state1, output1, state2 );
                        state1 = state2;
                        output1 = output2;
                        ++i;
                        output2 = B2-> B4_output[ i ];
                    }
                    A = A_add( A, state1, output1, next_c );
                }
            }
            ++p1;
        }
...
A_OBJECT AB_comp( A_OBJECT A1, B4_OBJECT B2, T2_OBJECT T2_Sigma )
{
    ...
    if ( A1-> A_nT == 1 ) {
        return( AB_comp_1( A1, B2, T2_Sigma ) );
    }
