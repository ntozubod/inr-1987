...
void A_st_free()
{
    int i;
    if ( st_work == 0 ) return;
    if ( st_ptr != st_work ) Sfree( (char *) st_ptr );
    for( i = 0; i < st_c_work; i++ ) Sfree( (char *) st_work[i] );
    Sfree( (char *) st_work );
    st_ptr = st_work = NULL;
}
...
int A_st_DFS( int q, int i )
{
    ...
    if ( i < st_len[q] ) return( st_work[q][i] );
    else if ( st_closed[q] ) return( MAXSHORT );

    if ( i > 0 && A_st_DFS( q, i - 1 ) == MAXSHORT ) return( MAXSHORT );

    try = WILD;
    for( p = GAs-> A_p[q]; p < GAs-> A_p[q+1]; ++p ) {
        if ( p-> A_b == 0 )     newtry = A_st_DFS( p-> A_c, i );
        else if ( i > 0 )       newtry = A_st_DFS( p-> A_c, i - 1 );
        else                    newtry = p-> A_b;
    ...
    if ( i != st_len[q] - 1 ) Error( "A_stems: Bounds check" );
    st_work[q][i] = try;
    return( try );
}
...
SHORT ** A_stems( A_OBJECT A, int tape )
{
    ...
    if ( A == NULL ) Error( "A_stems: No OBJECT" );
    ...
    if ( Aw-> A_mode < NFA_EQLAM ) Aw = A_lameq( Aw );
...
void A_prstems( A_OBJECT A, Tn_OBJECT Tn, int tape )
{
    int     q, i, ch;
    char *  s;
