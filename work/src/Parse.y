...
| NAME EQUAL reg_0 SEMI
{
    ...
    if ( (i = Tn_member( TAlist, $1, strlen( $1 ) )) >= 0 )
        A_destroy( Alist[ i ] );
    Alist[ Tn_insert( TAlist, $1, strlen( $1 ) ) ] = A;
    if ( Tn_member( TT, $1, strlen( $1 ) ) >= 0 )
        fprintf( fpout,
...
| NAME
{
    t = $1;
    if ( t[1] == '.' && t[0] >= '0' && t[0] <= '9' )
        $$ = A_letter( t[0] - '0', Tn_insert( TT, t + 2, strlen( t + 2 ) ) );
    else if ( (i = Tn_member( TAlist, t, strlen( t ) )) >= 0
              && Tn_member( TT, t, strlen( t ) ) < 0 )
        $$ = A_copy( Alist[ i ] );
    else {
